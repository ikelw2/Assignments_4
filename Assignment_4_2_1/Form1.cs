...
namespace Assignment_4_2_1
{
    ...
    public partial class Form1 : Form
    {
        ...
        {
            // EVENT HANDLER CODE - Tactic 1 of 2 // Use "pattern matching syntax" to cast the sender safely, or skips block if fails
            if (sender is TextBox txtOperand1) 
            {
                MyValidation myValidation = new MyValidation();
                if (myValidation.OperandValidated(txtOperand1.Text, out double Op1)) // learning point for me - must use Op1 within this block... 
                {
                    // if operand1 is validated
        ...
        //------------------------------
        // EVENT HANDLER - activates whenever user edits text in TextBox txtOperand1 
        private void txtOperand2_TextChanged(object sender, EventArgs e) // event handler
        {
            // EVENT HANDLER CODE - Tactic 2 of 2 // attempts to "convert sender to a TextBox" to cast the sender safely, or null if fails
